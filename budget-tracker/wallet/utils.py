...
class SplitTransactionUtils:
    ...
    def get_user_payable_amount(user_id, split):
        ...
        paid_amount = Transaction.objects.filter(user=user_id, split_expense=split).exclude(
            category=TransactionCategories.Income.value).aggregate(Sum('amount'))['amount__sum'] or 0
...
class TransactionUtils:
    ...
    def get_new_account_balance(prev_balance, amount, type):
        amount = amount if type == TransactionCategories.Income.value else -amount
